...
def lambda_handler(event, context):
    ...
    try:
        ...
        response = sns_client.publish(Subject="SUCCESS - Doordash Daily Data Processing", TargetArn=sns_arn, Message=message,
                                      MessageStructure='text')

    except Exception as err:
        print(err)
        message = "Input Doordash S3 File {} processing is Failed !!".format("s3://" + bucket + "/" + key)
        respone = sns_client.publish(Subject="FAILED - Daily Data Processing", TargetArn=sns_arn, Message=message,
                                     MessageStructure='text')
